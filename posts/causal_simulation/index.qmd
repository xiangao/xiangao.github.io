...
description: "Simulating data from a specified causal model using the frugal parameterization of Evans and Didelez (2024), with worked examples in the causl package."
---
...
The usual causal inference is to have observational data, then assume causal structure, then estimate the parameters.  This paper is about the opposite:  Suppose we have a causal model, what kind of distribution would generate it?
...
```{r dag1, include=TRUE, warning=FALSE,  cache=TRUE, message=FALSE, echo=FALSE, fig.alt='Directed acyclic graph with covariates X, treatment A, and outcome Y'}
library(ggplot2)
library(ggdag)
...
```{r dag2, include=TRUE, warning=FALSE,  cache=FALSE, message=FALSE, echo=TRUE, fig.alt='Directed acyclic graph for the smoking and birthweight example, with a latent confounder'}
library(ggplot2)
library(ggdag)
