...
Unified Julia front-end for causal effect estimation. Provides a single `estimate(...)` interface for typed estimands such as ATE and ATT, with TMLE and AIPW backends plus graph-implied backdoor adjustment through `CausalGraphs.jl`. Intended to consolidate and extend the roles of `NPCausal.jl` and `TMLE.jl`.
